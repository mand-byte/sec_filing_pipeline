...
from src.db.models import DelistedRouteCompletion, FilingAttempt, FilingStatus, PipelineLog, RouteWatermark
...
class ProductionRepository:
    """Persist production state without run-scoped audit history."""
    ...
    def get_route_watermark(self, cik: str, route: RouteName) -> datetime | None:
        ...
        return None if row is None else row.last_accepted_at
    ...
    def upsert_route_watermark(self, *, cik: str, route: RouteName, accepted_at: datetime) -> None:
        ...
        if self._is_postgresql():
            ...
            elif row.last_accepted_at is None or _normalize_to_utc(accepted_at) > _normalize_to_utc(row.last_accepted_at):
                row.last_accepted_at = accepted_at
                row.updated_at = now
        self._commit_with_rollback()
    ...
    def write_log(self, **_: object) -> None:
        """Production flow omits run-scoped DB logs; artifacts/console provide traceability."""
        return None
    ...
    def upsert_filing_attempt(
        ...
        *,
        ...
        now = datetime.now(timezone.utc)
        row = self.session.scalar(
            select(FilingStatus).where(
                FilingStatus.route == route,
                FilingStatus.accession_no == accession_no,
            )
        )
        if row is None:
            row = FilingStatus(
                ...
                error_detail=error_detail,
                first_seen_at=now,
                last_started_at=now if status == "in_progress" else None,
                last_completed_at=now if status == "completed" else None,
                updated_at=now,
            )
    ...
    def latest_filing_attempt(self, *, route: RouteName, accession_no: str) -> FilingStatus | None:
        return self.session.scalar(
            select(FilingStatus).where(
                FilingStatus.route == route,
                FilingStatus.accession_no == accession_no,
